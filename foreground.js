...
window.onload = async function() {
    ...
    if (!window.localStorage.getItem("jb_OM")) window.localStorage.setItem("jb_OM", 0);
    if (!window.localStorage.getItem("jb_BL")) window.localStorage.setItem("jb_BL", "");
    if (!window.localStorage.getItem("jb_BLVER")) window.localStorage.setItem("jb_BLVER", 0);

    if (window.localStorage.getItem("jb_OM") == "0") {
        let response = await browser.runtime.sendMessage({ command: "list" });
...
async function reportProcedure(event) {
    if (!reportCheck(document.location.pathname.slice(1))) {
        let response = await browser.runtime.sendMessage({ command: "report", user: document.location.pathname.slice(1) });
        repList = response;
        reportButton.innerHTML = "Sent For Review";
        reportButton.style.backgroundColor = "green";
        reportButton.style.cursor = "";
